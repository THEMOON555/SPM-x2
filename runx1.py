...
# [+]-------------------------------------------[+]
#  | Creator : DEMONX  		         |
#  | Github : https://github.com/THEMOON555    |
...
try:
    ...
    print (f"the module is not installed, install it first ")
    sys.exit()
...
def countdown(time_sec):
	try:
		while time_sec:
			...
			time.sleep(1)
			time_sec -= 1
		print (f"{W}[{Y}•{W}] start re-spaming....           ")
		time.sleep(5)
...
def tanya():
    ...
    if a == "y" or a == "Y":
        start()
    if a == "t" or a == "T":
        print (f"{W}[{R}!{W}] successfully exited the tool {R}!!!{W}")
        sys.exit()
    if a == "" or a == " ":
        print (f"{W}[{R}!{W}] Enter the correct choice {R}!!!{W}")
        sys.exit
...
def jam(): # Don't Remove Code !!!!
    try:
        ...
        banner()
        nomor = input(f"{W}[{R}?{W}] Target number  {Y}:{G} ")
        while True: # Looping
...
def normal(): # Don't Remove Code !!!!
        ...
        banner()
        nomor = input(f"{W}[{R}?{W}] Target Number {Y}:{G} ")
        jumlah = int(input(f"{W}[{R}?{W}] Amount Spam {Y}:{G} "))
...
def start(): # Def Untuk Start Tools
    ...
    if pil == "3":
        ...
        print (f"{Y}${W} Thank you for submitting the error, the creator will fix it soon")
        tanya()
    if pil == "4":
...
# Copyright 09/07/22
# Made By Executed Team ❤️ With DEMONX